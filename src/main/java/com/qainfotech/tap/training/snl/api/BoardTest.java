...
public class BoardTest {
	...
	public void loadboard() throws FileNotFoundException, UnsupportedEncodingException, IOException {
	   ...
	   assertThat(board.registerPlayer("user 2").length()).isEqualTo(2);
	   assertThat(board.registerPlayer("user 3").length()).isEqualTo(3);
	   assertThat(board.registerPlayer("user 4").length()).isEqualTo(4); UUID
	   uuid = null; JSONArray playerArray = new JSONArray(); playerArray =
	   board.getData().getJSONArray("players");
	   
	   for (int i = 0; i < playerArray.length(); i++) { JSONObject singlePlayer
	   = playerArray.getJSONObject(i); if
	   (singlePlayer.get("name").equals("user 2")) {
